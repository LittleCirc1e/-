...
def get_optimizer(name="Adam", model=None, lr=0.0002):
    if name == "Adam":
        ...
        return torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()),
                                 lr=lr, betas=(0.9,0.999), weight_decay=1e-6)
...
def path_provider(mode, tmode):
    assert mode in ['original', 'distance', 'nn']
    if os.path.exists("/home/jianshuo/dx"):
        root = "/home/jianshuo/dx"
    else:
        root = "/home/ubuntu/qiufeng/dx"
    if mode == 'original':
        d_path =  os.path.join(root, "ffpp")
    elif mode == "distance":
        d_path = os.path.join(root, "ffpp_distance")
    else:
        d_path = ""

    if tmode == 'distance':
        test_path = os.path.join(root, "ffpp_distance")
    else:
        test_path = os.path.join(root, "ffpp")

    return d_path, test_path, os.path.join(root, "logs")
